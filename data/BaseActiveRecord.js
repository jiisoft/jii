...
class BaseActiveRecord extends Model {
     ...
     */
    init() {
        this.trigger(BaseActiveRecord.EVENT_INIT);
    ...
    on(name, handler, data, isAppend) {
        ...
        var collectionFormat = this._detectKeyFormatCollection(name, BaseActiveRecord.EVENT_CHANGE_NAME);
        if (collectionFormat) {
            var collEventName = collectionFormat.subName || BaseActiveRecord.EVENT_CHANGE;
            collectionFormat.model.on(collEventName, handler, data, isAppend);
            return;
        }

        // Sub models support: foo.bar
        var modelFormat = this._detectKeyFormatModel(name, BaseActiveRecord.EVENT_CHANGE_NAME);
        if (modelFormat) {
            this._relatedEvents[modelFormat.name] = this._relatedEvents[modelFormat.name] || [];
            this._relatedEvents[modelFormat.name].push([
                modelFormat.subName,
                handler,
                data,
                isAppend
        ...
        // Relation support
        var relationFormat = this._detectKeyFormatRelation(name, BaseActiveRecord.EVENT_CHANGE_NAME);
        if (relationFormat) {
            var relationEvent = relationFormat.multiple ? Collection.EVENT_CHANGE : BaseActiveRecord.EVENT_CHANGE;

            this._relatedEvents[relationFormat.name] = this._relatedEvents[relationFormat.name] || [];
            this._relatedEvents[relationFormat.name].push([
                relationEvent,
                handler,
                data,
                isAppend
    ...
    _setRelated(name, value) {
        ...
        this.trigger(BaseActiveRecord.EVENT_CHANGE_NAME + name, new ChangeAttributeEvent({
            attribute: name,
            oldValue: null,
            newValue: value,
            isRelation: true
    ...
    _removeRelated(name) {
        ...
        oldValue.off(BaseActiveRecord.EVENT_CHANGE, {
    ...
    updateAttributes(attributes) {
        ...
        return this.constructor.updateAll(values, oldPrimaryKey).then(rows => {
    ...
    _updateInternal(attributes) {
        ...
        return this.beforeSave(false).then(bool => {
            ...
            return this.constructor.getDb().createCommand().updateModel(this, values);
        }).then(rows => {
    ...
    updateCounters(counters) {
        var oldPrimaryKey = this.getOldPrimaryKey(true);
        return this.constructor.updateAllCounters(_clone(counters), oldPrimaryKey).then(affectedRows => {
            if (affectedRows === 0) {
                return Promise.resolve(false);
            }
    ...
    delete() {
        return this.beforeDelete().then(bool => {
            ...
            return this.constructor.deleteAll(condition);
        }).then(result => {
    ...
    beforeSave(insert) {
        ...
        this.trigger(insert ? BaseActiveRecord.EVENT_BEFORE_INSERT : BaseActiveRecord.EVENT_BEFORE_UPDATE, event);
    ...
    beforeDelete() {
        var event = new ModelEvent();
        this.trigger(BaseActiveRecord.EVENT_BEFORE_DELETE, event);
    ...
    refresh() {
        ...
        return this.constructor.findOne(primaryKey).then(record => {
            if (record === null) {
                return Promise.resolve(false);
            }
    ...
    getPrimaryKey(asArray) {
        ...
        var keys = this.constructor.primaryKey();
    ...
    link(name, model, extraColumns) {
        ...
        return Promise.resolve().then(() => {
            if (relation.getVia() !== null) {
                ...
                return this.constructor.getDb().createCommand().insert(viaTable, columns);
            }

            var p1 = model.constructor.isPrimaryKey(_keys(relation.link));
            var p2 = this.constructor.isPrimaryKey(_values(relation.link));
            if (p1 && p2) {
                if (this.isNewRecord() && model.isNewRecord()) {
    ...
    _bindModels(link, foreignModel, primaryModel) {
        _each(link, (pk, fk) => {
            ...
            if (_isArray(foreignModel.get(fk))) {
                // relation via array valued attribute
    ...
    _createRelatedCollection(relationName, items) {
        ...
        var collection = null;
        var rootCollection = this.constructor.getDb() ? this.constructor.getDb().getRootCollection(relation.modelClass) : null;
    ...
    _fetchRelationFromRoot(name) {
        ...
        if (affectedAttributes.length > 0) {
            rootCollection.on([Collection.EVENT_CHANGE].concat(_map(affectedAttributes, attribute => {
    ...
    _onChangeRelatedModel(event) {
        ...
        var relation = this.getRelation(name);
        var modelClassName = Jii.namespace(relation.modelClass);
        var rootCollection = this.constructor.getDb().getRootCollection(modelClassName);

 ...
 */
Jii.defineClass('Jii.base.Component', {
	...
	hasEventHandlers: function (name) {
		...
		return this._events && this._events[name] && this._events[name].length > 0;// || Event::hasHandlers(this, name);
	},
	...
	on: function (name, handler, data, isAppend) {
		data = data || null;
		isAppend = _.isUndefined(isAppend) ? true : isAppend;
	...
	off: function (name, handler) {
		...
		var newEvents = [];
		var isRemoved = false;
		_.each(this._events[name], _.bind(function(event, i) {
			if (event[0] !== handler) {
	...
	trigger: function (name, event) {
		...
		if (this._events && this._events[name]) {
			...
			var isStopped = false;
			_.each(this._events[name], function(event) {
				if (isStopped) {
					return;
				}

				event.data = event[1];
				event[0] = this.normalizeHandler(event[0]);
				event[0].callback.call(event[0].context, event);
	...
	attachBehaviors: function (behaviors) {
		...
		_.each(behaviors, _.bind(function(behavior, name) {
	...
	detachBehaviors: function () {
		...
		_.each(_.keys(this._behaviors), _.bind(this.detachBehavior, this));
	},
	...
	ensureBehaviors: function () {
		...
		this._behaviors = [];
		_.each(this.behaviors(), _.bind(function(behavior, name) {
			this._attachBehaviorInternal(name, behavior);
		}, this));
	},
	...
	proxyBehaviors: function () {
		_.each(this.behaviors(), _.bind(function(behavior, name) {
			var className = _.isString(behavior) ? behavior : behavior.className;
	...
	_proxyBehaviorInternal: function (behaviorName, className) {
		...
		while (true) {
			...
			for (var name in behaviorClass.prototype) {
				...
				// Skip properties
				if (!_.isFunction(behaviorClass.prototype[name])) {
					continue;
				}

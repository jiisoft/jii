	...
	__static: /** @lends Jii */{
		...
		getAlias(alias, throwException) {
			...
			if (Jii._.has(this.aliases, root)) {
				...
				var finedPath = null;
				Jii._.each(this.aliases[root], (path, name) => {
					var testAlias = alias + '/';
		...
		configure(object, config) {
			for (var key in config) {
				...
				if (!Jii._.isUndefined(object[key]) && !Jii._.isFunction(object[key])) {
					if (Jii.helpers.Util.isStrictObject(object[key]) && Jii.helpers.Util.isStrictObject(config[key])) {
            ...
            for (var i = 0, l = arguments.length; i < l; ++i) {
                ...
                for (var key in obj) {
                    if (obj.hasOwnProperty(key)) {
                        if (Jii._.isObject(obj[key]) && !Jii._.isArray(obj[key]) && !Jii._.isFunction(obj[key]) && !(obj instanceof Jii.base.Object)) {

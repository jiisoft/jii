...
Neatness.defineClass('Jii', {
	...
	__static: /** @lends Jii */{
		...
		createContext: function(config) {
			...
			config = Jii._.merge({}, this._contextConfig || {}, config);
		...
		setAlias: function (alias, path) {
			...
			if (path !== null) {
				path = alias.indexOf('@') !== 0 ? Jii._.string.rtrim(path, '/') : this.getAlias(path);
		...
		createObject: function (config) {
			...
			args.concat(Jii._.rest(arguments));
		...
		configure: function (object, config) {
			for (var key in config) {
				...
				var setter = 'set' + Jii._.string.capitalize(key);

				if (Jii._.isUndefined(object[key]) && !Jii._.isFunction(object[setter])) {

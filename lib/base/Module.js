...
Jii.defineClass('Jii.base.Module', /** @lends Jii.base.Module.prototype */{
     ...
     */
    start() {
        return Promise.all(Jii._.map(this._components, component => {
			return Jii._.isFunction(component.start) ? component.start() : null;
		})).then(() => {
			return Promise.all(Jii._.map(this._modules, module => {
				return module.start();
			}))
		});
    },
    ...
    stop() {
        var promises = [];
        Jii._.each(this._components, component => {
	 ...
	 */
	setModules(modules) {
		Jii._.each(modules, (moduleObject, id) => {

...
Jii.defineClass('Jii.helpers.Url', /** @lends Jii.helpers.Url.prototype */{
	...
	__static: /** @lends Jii.helpers.Url */{
        ...
        to: function(url, context, scheme) {
            if (Jii._.isObject(url)) {
        ...
        toRoute: function(route, context, scheme) {
            scheme = scheme || false;

            if (!Jii._.isObject(route)) {
                route = [route];
            }

            return scheme ?
                this.__static._getUrlManager().createAbsoluteUrl(route, context, Jii._.isString(scheme) ? scheme : null) :
         ...
         */
        parseRoute: function(route, context) {
            var params = {};

            // Format ['site/index', {foo: 'bar'}]
            if (Jii._.isArray(route)) {
                if (Jii._.isObject(route[1]) && route.length === 2) {
                    params = route[1];
                }
                route = route[0];
            }
        ...
        _normalizeRoute: function(route, context) {
            route = Jii.getAlias(route);
            if (route.substr(0, 1) === '/') {
                // absolute route
                return Jii._s.trim(route, '/');
            }

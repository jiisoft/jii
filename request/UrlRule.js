 ...
 * and add it to [[Jii.request.UrlManager::rules]] like this:
 *
 * ~~~
 * rules: {
 ...
 * ~~~
 *
 * @class Jii.request.UrlRule
 * @extends Jii.base.Object
 */
var UrlRule = Jii.defineClass('Jii.request.UrlRule', /** @lends Jii.request.UrlRule.prototype */{

	__extends: Object,

	__static: /** @lends Jii.request.UrlRule */{
	...
	createUrl(manager, route, params) {
		...
		if (route !== this.route) {
			...
			_each(this._routeParams, (token, name) => {
				tr[token] = !_has(this.defaults, name) || this.defaults[name] !== matches[0][name] ?
					matches[0][name] :
					'';
			});
		}
